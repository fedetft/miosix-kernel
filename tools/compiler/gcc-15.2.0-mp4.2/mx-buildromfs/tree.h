...
/**
 * Wrapper for lstat or workaround for Windows
 * \param path path to lstat
 * \param st struct stat to return stat result
 * \return 0 on success
 */
inline int lstatEntry(const std::filesystem::path& path, struct stat& st)
{
#ifndef WIN32
    ...
    try {
        auto status=symlink_status(path);
        st.st_mode=0777; //FIXME: file permissions
        if(is_regular_file(status)) st.st_mode |= S_IFREG;
        else if(is_directory(status)) st.st_mode |= S_IFDIR;
        else if(is_symlink(status)) st.st_mode |= S_IFLNK;
        st.st_uid=0; //FIXME: file user
        st.st_gid=0; //FIXME: file group
        return 0;
                        ...
                        const std::filesystem::path& directory, int uidGidOverride=-1)
{
    ...
    struct stat st;
    if(lstatEntry(directory,st)!=0 || (st.st_mode & S_IFMT) != S_IFDIR)
    {
    ...
    entry.name=directory.filename().u8string();
    ...
    for(directory_iterator it(directory);it!=end;++it)
    {
        ...
        if(is_directory(it->status()))
        {
            ...
            struct stat st;
            if(lstatEntry(it->path(),st)!=0) throw std::runtime_error("stat");
            ...
            subentry.name=it->path().filename().u8string();
