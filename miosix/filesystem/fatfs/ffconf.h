...
//Note by TFT: LFN is used in the following functions: f_open(),
...
// By Raul: We always want 64 bit LBA when exFAT is enabled, but when exFAT is disabled
// it is pointless to have it 64 bit wide. 
// Note that when exFAT is enabled FAT32 is also enabled, this means that FAT32 will use
// a 64 bit LBA, but this is not a problem since the LBA is just a sector number 
// and the sector size is still 512 bytes.
#define FF_LBA64		FF_FS_EXFAT
