/***************************************************************************
 *   Copyright (C) 2026 by Terraneo Federico, Caruso Nicolò, Radu Raul     *
 *                                                                         *
...
namespace miosix
{
    ...
    /**
     * Directory class for ExFatFs
     */
    class ExFatDirectory : public DirectoryBase
    {
    public:
        /**
    ...
    /**
     * Files of the exFAT filesystem
     */
    class ExFatFile : public FileBase
    {
    public:
        /**
         * Constructor
         ...
         */
        ExFatFile(intrusive_ref_ptr<FilesystemBase> parent, int flags, KernelMutex &mutex);
    ...
    int ExFatFs::lstat(StringPart &name, struct stat *pstat)
    {
        if (failed)
            return -ENOENT;
        memset(pstat, 0, sizeof(struct stat));
        ...
        info.lfsize = 0;
        if (int result = translateError(f_stat(&filesystem, name.c_str(), &info)))
            return result;

        pstat->st_ino = info.inode;
        pstat->st_mode = (info.fattrib & AM_DIR) ? S_IFDIR | 0755  // drwxr-xr-x
