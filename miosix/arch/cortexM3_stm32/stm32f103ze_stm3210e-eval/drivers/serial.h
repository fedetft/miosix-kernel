 ...
 * immediately. Calling read function will block the thread until serial is
 * re-enabled and a character is received.
...
/**
 * \internal
 * \return true if all tx buffers (software and hardware) are empty. This means
 * that all data has been sent. Cannot be used if the kernel is not started,
 * paused, or inside an IRQ.
 */
...
/**
 * \internal
 * Read a character from the serial port. Nonblocking.
 ...
 * lost flag. Cannot be used if the kernel is not started, paused, or inside an
 * IRQ.
 */
bool serialRxLost();
 ...
 * IRQ, and/or when the scheduler is not started or paused.
