...
void IRQbspInit()
{
    ...
    IRQsetDefaultConsole(intrusive_ref_ptr<Device>(
        new EFM32Serial(EFM32Serial::UART1_ID,defaultSerialSpeed,
                        tx::getPin(),rx::getPin(),2)));
}
...
void shutdown()
{
    ...
    //Sequence to enter EM4, not used because it prevents programming the
    //board once it's shut down
    //for(int i=0;i<5;i++)
    //{
    //    EMU->CTRL=2<<2;
    //    EMU->CTRL=3<<2;
