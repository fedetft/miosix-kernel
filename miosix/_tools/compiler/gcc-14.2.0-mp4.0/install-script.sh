...
extract 'gcc' $GCC.tar.xz patches/gcc.patch patches/libgomp-fix-crash.patch patches/libgomp_gcc14.patch
...
$SUDO rm "$DESTDIR$PREFIX/lib/gcc/arm-miosix-eabi/14.2.0"/*.o
$SUDO rm "$DESTDIR$PREFIX/lib/gcc/arm-miosix-eabi/14.2.0"/*.a
...
if [[ $DESTDIR ]]; then
	if [[ ( $(uname -s) == 'Linux' ) && ( $HOST == *linux* ) ]]; then
		...
		./$MAKESELF/makeself.sh --xz --keep-umask \
			$DESTDIR$PREFIX \
			MiosixToolchainInstaller14.2.0mp4.0.run \
			"Miosix toolchain for Linux (GCC 14.2.0-mp4.0)" \
			"./installer.sh"
		...
		productbuild \
			...
			--package-path ./ \
			"./MiosixToolchainInstaller14.2.0mp${__GCCPATCUR}_${suffix}.pkg"
	fi
else
